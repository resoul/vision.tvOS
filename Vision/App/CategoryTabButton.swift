...
final class CategoryTabButton: UIControl {
    ...
    private func updateLook(animated: Bool) {
        let block = {
            ...
            self.iconView.tintColor = self.isActiveTab ? .white : UIColor(white: 0.45, alpha: 1)
        ...
        if animated {
            UIView.animate(withDuration: 0.22, delay: 0, usingSpringWithDamping: 0.75, initialSpringVelocity: 0.5, animations: block)
        } else {
            block()
        }
    }
    ...
    override func didUpdateFocus(in context: UIFocusUpdateContext,
        ...
        coordinator.addCoordinatedAnimations({
            ...
            self.iconView.tintColor = self.isFocused ? .white
                : (self.isActiveTab ? .white : UIColor(white: 0.45, alpha: 1))
